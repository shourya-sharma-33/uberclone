...
Hello! I am Shourya Sharma, and this is a Full Stack Uber Clone App made using the MERN Stack (MongoDB, Express, React, and Node.js).  
[Will append more to the introduction after I complete the project, discussing all the features.]

## Note from the Developer

This project, including its codebase and documentation, has been created entirely through my own effort without the use of AI-generated code or automated writing tools. I built and documented this project from scratch as a way to sharpen my problem-solving skills, deepen my understanding of backend development, and strengthen my ability to communicate technical details clearly. Every decision, implementation, and explanation here reflects my own learning process and hands-on work.  
In my initial commits, I had generated documentation using AI, but later I deleted it and decided to rewrite it myself while testing and creating flowcharts for each API endpoint.
...
| `password`                | String  | Yes      | Hashed before storage, excluded from queries. |
...
- `comparePassword(password)` → Compares plain password with stored hashed password.  
- `hashPassword(password)` → Returns bcrypt hash of the password.

### Blacklisted Token Model

An extra layer of security to prevent users from logging in with old or compromised tokens that may still be stored on their system. This collection stores all previously used tokens and ensures any presented token is not blacklisted (already used or invalidated).

| Field       | Type  | Required | Details                 |
|-------------|-------|----------|-------------------------|
| `token`     | String| Yes      | Stores all past tokens.  |
| `createdAt` | Date  | Yes      | Default is creation time.|
...
File: `middlewares/auth.middleware.js` - contains authentication based middlewares.

#### authUser Middleware

The `authUser` middleware authenticates a user based on their **JWT token**. It follows these steps:
...
5. Calls `next()` if authorized, or returns a 401 Unauthorized error otherwise.
...
Occurs when the request lacks a valid authentication token in `cookies.token` or `Authorization` header.
...
Occurs when the provided token is found in the `blacklistToken` collection (e.g., after logout).
...
If database queries fail (e.g., user not found due to DB outage), it could return:
...
The `authCaptain` middleware authenticates a captain based on their **Token**. It follows these steps:
...
The `authCaptain` middleware can return the following errors:
...
If database queries fail (e.g., captain not found due to DB outage), this could return:
...
File: `services/user.services.js`

#### `createUser`

- Takes `firstname`, `lastname`, `email`, and `password` and saves them in the database collection.
...
- Takes `firstname`, `lastname`, `email`, `password`, `color`, `plate`, `model`, `capacity`, and `vehicleType` and saves them in the database collection.
...
- Check if:
  ...
  - **Firstname** should be a minimum of 3 letters
  - **Password** must be a minimum of 6 letters
- Run `userController.registerUser` controller

##### `userController.registerUser`

- Store validation results in the `errors` object
- Return if the `errors` object is not empty
- Extract `fullname`, `email`, and `password` from `req.body`
- Find `user` document from the `user` collection in the database
- If `user` exists, then return with the message `user already exists`
- Hash the password using the `hashPassword` method in the user model
- Save `user` in the database using the `createUser` service
- Generate a token using the `generateAuthToken` method in the user model
- Send a response with the `token` and `user` document
...
Occurs when the request body fails validation rules set via `express-validator`.  
